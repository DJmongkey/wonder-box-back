...
exports.postBaseInfo = async (req, res, next) => {
  ...
  try {
    ...
    if ( !(diffDay >= 2 &&  startDate < endDate) ) {
      return next(new HttpError(400, "시작일은 종료일보다 과거여야 합니다(최소 2일이상)"))
    }
    ...
    const calendar = await Calendar.create({title,creator, startDate, endDate, options});

    if (!calendar) {
      return next(new HttpError(400, ERRORS.CALENDAR.NOT_FOUND_BASE_INFO));
    }

    await User.updateOne({ _id: userId }, { $addToSet: { calendars: calendar._id } });

    return res.status(201).json({ result: "ok", calendarId: calendar._id , message: "새로운 캘린더에 기본정보가 저장 되었습니다."});
  } catch (error) {
    console.error(error)
    return next(new HttpError(500, ERRORS.INTERNAL_SERVER_ERR));
  }
